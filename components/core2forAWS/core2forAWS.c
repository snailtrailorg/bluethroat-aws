...
void Core2ForAWS_Init(void) {    
    ...
    Core2ForAWS_Display_SetBrightness(0);
...
#define DISPLAY_BRIGHTNESS_MIN_VOLT 2350
#define DISPLAY_BRIGHTNESS_MAX_VOLT 3150
...
void Core2ForAWS_Display_SetBrightness(uint8_t brightness) {
    ...
    uint32_t max_volt_square = DISPLAY_BRIGHTNESS_MAX_VOLT * DISPLAY_BRIGHTNESS_MAX_VOLT;
    uint32_t min_volt_square = DISPLAY_BRIGHTNESS_MIN_VOLT * DISPLAY_BRIGHTNESS_MIN_VOLT;
    uint32_t volt_square = (uint32_t)brightness * (max_volt_square - min_volt_square) / 100 + min_volt_square;
    uint16_t volt = (uint16_t)sqrt((double)volt_square);
