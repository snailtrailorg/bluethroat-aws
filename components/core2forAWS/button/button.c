...
void Button_SetLongPressTime(Button_t* button, uint32_t million_second) {
    xSemaphoreTake(button_lock, portMAX_DELAY);
    button->long_press_time = pdMS_TO_TICKS(million_second);
    xSemaphoreGive(button_lock);
}

uint8_t Button_WasLongPress(Button_t* button) {
    xSemaphoreTake(button_lock, portMAX_DELAY);
...
void Button_Update(Button_t* button, uint8_t press, uint16_t x, uint16_t y) {
    ...
    if (value != button->last_value) {
        ...
        if (value && button->long_press_time && (now_ticks - button->last_press_time > button->long_press_time)) {
            button->state |= LONGPRESS;
            button->last_press_time = now_ticks;
        }
    }
